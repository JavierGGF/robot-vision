...
def create_calibration(
    ...
    Dimensions refer to the reference rectangle:
    width_mm: left-to-right distance.
    height_mm: top-to-bottom distance.
    """
    image_points = np.asarray(image_points, dtype=np.float32)

    if image_points.shape != (4, 2):
        raise ValueError("Exactly four points with X and Y are required.")

    if not np.isfinite(image_points).all():
        raise ValueError("Point coordinates must be finite.")

    if (
        not np.isfinite([width_mm, height_mm]).all()
        or width_mm <= 0
        or height_mm <= 0
    ):
    ...
    if not cv2.isContourConvex(contour):
        raise ValueError(
            "Select four distinct corners in the specified order."
        )
    ...
    return cv2.getPerspectiveTransform(image_points, reference_points)


def pixel_to_mm(x, y, matrix):
    """Convert an image point to coordinates on the reference plane."""
